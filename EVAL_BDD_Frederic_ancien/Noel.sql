...
CREATE TABLE Reservation(
   ...
   date_debut DATE,
   date_fin DATE,
   est_confirmée BOOLEAN,
   prix_total_ttc DECIMAL(15,2)  ,
   prix_total_ht VARCHAR(50) ,
   est_payée VARCHAR(50) ,
...
ADD CONSTAINT FK_Reservation_client       FOREIGN KEY(Id_Client) REFERENCES Client(Id_Client),
ADD CONSTAINT FK_Reservation_Acompte      FOREIGN KEY(Id_Acompte) REFERENCES Acompte(Id_Acompte),
ADD CONSTAINT FK_Reservation_type         FOREIGN KEY(Id_Type_reservation) REFERENCES Type_reservation(Id_Type_reservation),
ADD CONSTRAIN FK_Reservation_options      FOREIGN KEY(Id_Options) REFERENCES Options(Id_Options);
